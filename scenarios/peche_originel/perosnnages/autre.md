> [!NOTE]
> Vous ne faite pas parti de cette tribu, vous êtes autre chose. 
> Une créature de la nuit, vous préférez l'obscurité à la lumière. 
> La tribu vous a accepté enfant et vous avez appris a vivre parmi eux.

# L'autre
- Nom: #TBD 
- Apparence: #TBD
- Espèce: *Peuple de la nuit*
## Statistiques
- **Charme** : 0
- **Cool** : -1
- **Futé**: +2
- **Coriace**: -1
- **Bizarre**: +3
## Blessures
`[ ][ ][ ]|[ ][ ][ ][ ]`   
*instable* `[ ]`
## Faveur ancestrale ( Chance )
`[ ][ ][ ]`
## Malédiction
- **Devoir** *Communier avec la nuit* : Vous devez chaque nuit communier avec l'obscurité et passer du temps sous le ciel de la nuit.
## Manœuvres
### Nyctalope
Vous pouvez voir dans l'obscurité comme en plein jour
### Force surnaturelle
Lancez **+Bizarre** au lieu de **Coriace** lorsque vous **cassez la gueule a quelqu'un**
## Attaques naturelles
- **Griffes** ( 3 dégâts, allonge )
- **Absorption d'énergie** ( 1 dégât, personnel, absorption d’énergie )
## Equipement
3 Equipements vêtements compris ( p 375 )



